...
fn get_transcript(episode: &Episode) -> Result<(), &'static str> {
    ...
    if std::path::Path::new(&episode.transcript_location).exists() {
        println!("Transcript already exists for {}", episode.transcript_location);
        return Ok(());
    }
...
pub async fn get_episodes() -> Vec<Episode> {
    ...
    for entry in rss_feed.entries.clone() {
        ...
        let thumbnail_url = entry.media.first().unwrap().thumbnails.first().unwrap().image.uri.clone();
...
pub async fn main()  {
    ...
    let limit = 60;
    download_episodes_from(&list_of_episodes[0..limit].to_vec());

    let second_episode = &list_of_episodes[1];
    println!("Second episode: {:?}", second_episode);

    for episode in list_of_episodes[0..limit].iter() {
